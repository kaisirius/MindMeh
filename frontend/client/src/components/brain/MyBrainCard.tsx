...
const BrainCard =  ({ imageURL, title, hash }: T_brainCardProps) => {
  const { CurrentBrains, setCurrentBrains } = useCurrentBrainsContext();
  const navigate = useNavigate();
  const handleDeleteBrain = async () => {
    try{
      await api.delete(`/api/v1/brain/${hash}`)
      const UpdatedCurrentBrains = CurrentBrains.filter(b => b.hash !== hash);
      toast.info("Brain deleted")
      setCurrentBrains(UpdatedCurrentBrains)
    } catch(err) {
      if(axios.isAxiosError(err)) {
        if(err.response?.status === 400) {
          toast.info("Unauthorized to delete the brain")
        } 
        navigate("/error")
      }
    }
